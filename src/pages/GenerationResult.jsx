...
export default function GenerationResult() {
  ...
  useEffect(() => {
    ...
    async function loadGeneration() {
      try {
        ...
        setDownloadFormat((current) => current || (nextGeneration?.outputFormat || 'pdf'))
        setError('')
  ...
  const isSingle = Boolean(generation && (generation.mode === 'single' || !generation.dataFileId))
  ...
  async function handleDownloadAll() {
    ...
    try {
      if (isSingle) {
        const response = await generationService.downloadSingle(generation._id, 0, downloadFormat)
        downloadBlob(response.data, `certificate-${generation._id}.${formatExtension(downloadFormat)}`)
      } else {
        const response = await generationService.downloadArchive(generation._id, downloadFormat)
        downloadBlob(response.data, `certificates-${generation._id}-${downloadFormat}.zip`)
      }
  ...
  async function handleDownloadFile(file) {
    ...
    setError('')
    try {
      const response = await generationService.downloadSingle(generation._id, file.recordIndex || 0, downloadFormat)
      downloadBlob(response.data, `certificate-${(file.recordIndex || 0) + 1}.${formatExtension(downloadFormat)}`)
    } catch {
  ...
  return (
    <>
      <PageHeader
        ...
        title={generation ? (isSingle ? 'Certificate ready' : `Batch ${generation.status}`) : 'Generation result'}
        ...
        actions={(
          <>
            ...
            <Button onClick={handleDownloadAll} disabled={!files.length}>{downloadStatus || (isSingle ? 'Download certificate' : 'Download all')}</Button>
            <Button as={Link} to={isSingle ? `/templates/${templateId}/editor` : '/generate'} variant="secondary">Generate again</Button>
          </>
        )}
      />
      ...
      <Card className="result-card">
        ...
        <h2>{generation?.status === 'completed' ? (isSingle ? 'Certificate ready' : 'Certificates ready') : generation?.status || 'Loading result'}</h2>
